    ...
    messages: [
      {
        ...
        content: [
          ...
          '',
          '硬性禁止编造规则：',
          '1. 如果输入里没有明确图片描述，禁止写任何具体画面。',
          '2. 禁止自行添加服装、动作、道具、场景、光线、表情、成员互动、时间、地点。',
          '3. 禁止写“照片里”“那张”“第一张”“第二张”“最后一张”“六宫格”等具体图片描述。',
          '4. 禁止写未经输入提供的信息：西瓜、白T、牛仔裤、牛仔短裤、短裤、小扇子、阳光、夕阳、台阶、棒球帽、比耶、合照、成员合照。',
          '5. 如果只收到关键词，只能使用安全表达：这次更新、这组近照、这组图、状态、气质、风格、账号观察、整体观感。',
          '',
          '硬性禁止口吻：',
          '禁止使用：姐妹们、啊啊啊、呜呜呜、美到失语、绝了、封神、鲨疯了、我先磕为敬、姐姐娶我、快来评论区告诉我、点赞在看、嘿嘿。',
          '禁止使用微博/小红书 hashtag。禁止硬性互动 CTA。不要堆叠感叹号。',
          '',
          '硬性输出格式：',
          '输出必须是 JSON 对象：{"title":"标题","markdown":"正文"}。',
          'title 放标题；markdown 只放正文，不要重复标题。正文不要出现 Markdown 标题符号 #。正文 300-420 字，短段落，适合微信公众号阅读。',
          '',
          '生成前自检：',
          '在最终输出前，检查正文是否出现任何违禁词、违禁细节、Markdown 标题符号、hashtag、硬性互动 CTA，或任何未经用户输入确认的具体画面。只要出现，必须重写正文后再输出。'
        ].join('\n')
      },
      {
        role: 'user',
        content: [
          ...
          '- 如果关键词里没有明确图片描述，不要写任何具体画面、服装、动作、道具、场景、光线、表情或成员互动。',
          '- 开头直接进入观察，不要用微博/小红书式口吻。',
          '- 结尾要有余味，可以是状态判断、气质总结或读者共鸣，不要硬互动。',
          '- 避开未证实恋情、爆料、引战、身体羞辱、整容猜测、私生活攻击。',
          '',
          '再次强调：markdown 字段只能是正文，不要重复标题，不要 #，不要 hashtag。'
        ].join('\n')
      }
    ],
    response_format: { type: 'json_object' }
